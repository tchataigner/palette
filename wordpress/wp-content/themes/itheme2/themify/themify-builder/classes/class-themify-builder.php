...
if (!class_exists('Themify_Builder')) {
	...
	class Themify_Builder {
		...
		function post_type_editor_support_check( $active ) {
			...
			if( ! defined( 'DOING_AJAX' ) /* check for Ajax requests, this prevents Builder frontend editor not loading via Ajax */
				&& ! in_array( $post_type, $this->builder_post_types_support() )
			) {
				$active = false;
			}
		...
		function get_modules($select = 'all') {
			...
			foreach ($_modules as $value) {
				if ( is_dir( $value ) ) continue; /* clean-up, make sure no directories is included in the list */
					...
					switch ($script) {
						...
						case 'themify-builder-front-ui-js':
							...
							wp_register_script( $script, THEMIFY_BUILDER_URI . "/js/themify.builder.front.ui.js",
								array( 'jquery', 'jquery-ui-tabs', 'themify-builder-common-js', 'jss' ), THEMIFY_VERSION, true );
							wp_enqueue_script($script);
			...
			<?php endif; ?>
				...
				function write_stylesheet($data_set) {
					...
					if (!empty($css_to_save)) {
						...
						if ($write = $wp_filesystem->put_contents($css_file, $css_to_save, FS_CHMOD_FILE)) {
