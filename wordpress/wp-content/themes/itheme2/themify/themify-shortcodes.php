...
function themify_shortcodes_js_css() {
	...
	$themify_twitter_instance = 0;
		...
		if(!wp_script_is('themify-main-script')){
			...
			wp_localize_script('themify-main-script', 'themify_vars',array('version'=>THEMIFY_VERSION,'url'=>THEMIFY_URI));
	...
	switch ( $code ) {
		...
		case 'themify_map':
			...
			return '<div class="shortcode map">
									<div data-map="'.esc_attr(json_encode($data)).'" id="themify_map_canvas_' . esc_attr( $num ) . '" style="display: block;width:' . esc_attr( $width ) . ';height:' . esc_attr( $height ) . ';" class="map-container themify_map"></div>
								</div>';
		break;
		case 'video':
		case 'themify_video':
